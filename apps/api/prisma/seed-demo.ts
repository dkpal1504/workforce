...
async function main() {
  ...
  console.log("Demo seed: full demonstration data (projects, WBS, UoM, networks, Job Orders, employees, bookings).\nFor the production-like starting point use `npm run db:seed` instead.");
  ...
  // --- Networks: validated reference, ONE WBS element each (SAP-fed in phase 2)
  // A Network belongs to a WBS of its project, never to the whole project: one Network
  // number cannot span two WBS rows, so each row below names the WBS it belongs to.
  const networkRows = [
    { projectCode: "PRJ-A", wbsCode: "A.HULL.0010.100", code: "SAP-NW-91001", name: "Hull networks" },
    { projectCode: "PRJ-A", wbsCode: "A.OUTF.0020.100", code: "SAP-NW-91002", name: "Outfit networks" },
    { projectCode: "PRJ-B", wbsCode: "B.HULL.0020.150", code: "SAP-NW-92001", name: "Block 223 networks" },
    { projectCode: "PRJ-C", wbsCode: "C.SFR.0045.201", code: "SAP-NW-93001", name: "Surface treatment networks" },
    { projectCode: "PRJ-C", wbsCode: "C.REP.0045.202", code: "SAP-NW-93002", name: "Repair networks" },
    { projectCode: "PRJ-D", wbsCode: "D.REP.0030.110", code: "SAP-NW-94001", name: "Repair networks" },
    { projectCode: "PRJ-N", wbsCode: "GENERAL", code: "DUMMY", name: "Standing / idle hours (no SAP network)" },
  ];
  const networkId: Record<string, number> = {};
  for (const r of networkRows) {
    const created = await prisma.network.create({
      data: {
        projectId: projectFor(r.projectCode),
        wbsId: wbsId[`${r.projectCode}:${r.wbsCode}`],
        code: r.code,
        name: r.name,
      },
    });

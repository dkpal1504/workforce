...
| `403 ORIGIN_NOT_ALLOWED` ("This origin is not allowed to call the API.") after switching to a hostname or adding a port | an origin is `scheme://host:PORT`; the browser's exact value is not in `CORS_ORIGINS` (a bare `http://host` never matches `http://host:8099`) | list the exact origin with its port and `up -d --force-recreate api`. The web image's nginx now forwards `Host $http_host`, so the API also accepts its own origin with the port and no longer needs every host:port listed |
| Login (or any save) answers **500 Internal server error** while the pages themselves load | the browser's origin is missing from `CORS_ORIGINS`: browsers send an `Origin` header on POST/PUT/PATCH/DELETE even for same-origin calls, the old CORS middleware threw, and the production error handler hid the reason | put **every origin users type** in the list (comma-separated, no trailing slash) - e.g. `CORS_ORIGINS=http://10.5.1.193:8099,https://workforce.swan.co.in` - then `up -d --force-recreate api`. Same-origin calls are now always allowed, and a foreign origin answers `403 ORIGIN_NOT_ALLOWED` with a warning in the api log instead of a 500 |
...
| `Prisma failed to detect the libssl/openssl version` then `Can't write to /app/node_modules/@prisma/engines` | the slim Node base image ships no `openssl` CLI, so Prisma picked the `openssl-1.1.x` engine and tried to download it as the unprivileged `node` user | fixed in the Dockerfile (openssl installed in the build, migrate and api stages; engine directories chowned to `node`). Rebuild after pulling |
| A stale folder under `prisma/migrations` (untracked leftovers from an older checkout) breaks `migrate deploy` | every folder in that directory is applied, in filename order, from the `migrate` image built out of the working tree | remove the untracked folders, verify with `git status --short`, rebuild the migrate image |
